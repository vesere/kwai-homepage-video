...
let userId = "3xi8vrehcwr2ece"

let pcursor = "1.671031332916E12"
...
function dowload(){
    axios({
        ...
        result.map((item,index)=>{
            // 全部执行  下载函数
            setTimeout(()=>{
                wrmp4(item.photo.photoUrl,item.photo.caption)
            },Math.random()*10000)
...
function wrmp4(url,title){
    ...
    let file = Buffer.from(buf)
        ...
        // 一异步api不能使用try catch捕获错误 要在回调
        fs.writeFile(`./视频/${title}.mp4`,file,(err)=>{
            count++
